...
make(User, Server, Resource) ->
    case catch make_do(User, Server, Resource) of
...
make_do(User, Server, Resource) ->
    {
    case nodeprep(User) of
        error -> throw({error, "node error"});
        LUser -> LUser
    end,
    case nameprep(Server) of
        error -> throw({error, "name error"});
        LServer -> LServer
    end,
    case resourceprep(Resource) of
        error -> throw({error, "resource error"});
        LResource -> LResource
    end
    }.
    ...
    case binary:match(S, SplitPattern) of
        ...
        {Pos1, _} ->
            case binary:at(S, Pos1) of
                $/ ->
                    make(<<>>, binary:part(S, 0, Pos1), binary:part(S, Pos1+1, Size-Pos1-1));
                _ ->
                    Pos1N = Pos1+1,
...
make_test_() ->
    [ {"make jid",
        fun() ->
            ...
            Server   = <<"localhost">>,
            Resource = <<"resource">>,
            Excepted = #jid{user = User, server = Server, resource = Resource,
                            luser = User, lserver = Server, lresource = Resource},
            ?assertEqual(Excepted, make(User, Server, Resource)),
            ?assertEqual(Excepted, make({User, Server, Resource}))
        end}
    , {"split jid",
        fun() ->
            ...
            ?assertEqual({User, Server, Resource}, split(Jid)),
            ?assertEqual(error, split(error_jid))
        end}
    , {"from string",
        fun() ->
            ok = start_need(),
            ...
            Server   = <<"localhost">>,
            Resource = <<"resource">>,
            String = "alice@localhost/resource",
            Binary = erlang:list_to_binary(String),
            ?assertEqual(make({User, Server, Resource}), from_string(Binary)),
            ?assertEqual({error, need_jid_as_binary}, from_string(String))
        end}
    , {"to string",
        fun() ->
            ok = start_need(),
            ...
            Binary = erlang:list_to_binary(String),
            Jid = make(User, Server, Resource),
            ?assertEqual(Binary, to_string(Jid)),
            ?assertEqual(Binary, to_string({User, Server, Resource})),
            ?assertEqual(<<"alice@localhost">>, to_string({User, Server, <<"">>}))
        end}
    ].
